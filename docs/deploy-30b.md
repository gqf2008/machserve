...
> 注意：MACH_MODEL 只需指向任一 shard 文件名（server 会解析到该目录，loader 发现全部 shard）；同目录的 `config.json` / `tokenizer.json` 自动发现，`MACH_CONFIG` / `MACH_TOKENIZER` 可显式覆盖。启动/doctor 会先做 header+index 校验（缺失分片、截断、index 不一致在权重加载前报错）。Q4 要求 f16 计算路径（默认）。
