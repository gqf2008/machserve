...
/// decoding. This is the GPU counterpart of the CPU run-descriptor test.
#[test]
#[ignore = "controlled-power run-based paged attention regression; set MACH_TEST_PAGED_TILED_ATTN_RUNS_F32=1"]
fn batched_paged_tiled_f32_gqa_runs_match_reference_mixed_slots() {
    if std::env::var("MACH_TEST_PAGED_TILED_ATTN_RUNS_F32").as_deref() != Ok("1") {
        panic!(
            "MACH_TEST_PAGED_TILED_ATTN_RUNS_F32=1 is required for this controlled run-based test"
        );
    }
    let hip = hip_ctx().unwrap_or_else(|| {
        panic!("no HIP device is present; this test is opt-in and must not silently skip")
