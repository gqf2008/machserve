//! Reproducible A/B benchmark: f16 vs storage-FP8 weights on a real
//! checkpoint, measuring host RAM, TTFT, TPOT and the GPU logits gap.
//!
//! FP8 is a *storage* format: host weights stay packed E4M3 (1 byte/element,
//! ~2x smaller than f16 / ~4x smaller than f32) and are dequantized to f16
//! per tensor during upload, so the device compute path is identical to f16 —
//! the only expected difference is the E4M3 quantization error in the weights.
//!
...
//! MACH_FP8 (unset = run both legs + A/B, 0 = f16 only, 1 = FP8 only),
...
#[cfg(feature = "hip")]
use mach_model::{Config, Weights, WeightsFp8};
#[cfg(feature = "hip")]
use std::sync::Arc;
#[cfg(feature = "hip")]
...
fn main() {
    ...
    use mach_model::loader::{load_safetensors, load_safetensors_fp8};
    ...
    let capacity: usize = std::env::var("MACH_CAPACITY")
        ...
        .and_then(|v| v.parse().ok())
        .unwrap_or(4);
    let fp8_mode: i32 = std::env::var("MACH_FP8")
        .ok()
        .and_then(|v| v.parse().ok())
        .unwrap_or(-1);
    ...
    // ---- FP8 leg (packed E4M3 host weights -> f16 device compute) ----
    let mut fp8: Option<Meas> = None;
    if fp8_mode != 0 {
        let t0 = Instant::now();
        let wfp8: WeightsFp8 =
            load_safetensors_fp8(&model_path, &cfg, true).expect("load fp8 weights");
        let load_ms = t0.elapsed().as_secs_f64() * 1000.0;
        let host_ram_mb = (host_ram_bytes().saturating_sub(base_ram)) as f64 / 1e6;
        println!(
            "\n[fp8] loaded packed-E4M3 host weights in {load_ms:.0} ms; host RAM delta {:.2} GiB",
            gib(host_ram_mb * 1e6)
        );

        let ttft_ms = bench_ttft_fp8(&hip, cfg, &wfp8, prompt_len, capacity);
        let (tpot_ms, logits) = bench_tpot_fp8(&hip, cfg, &wfp8, batch, tokens);
        fp8 = Some(Meas {
            label: "fp8".into(),
            host_ram_mb,
            ttft_ms,
    ...
    for m in [&f16, &fp8].into_iter().flatten() {
    ...
    if f16.is_some() && fp8.is_some() {
        println!(
            "note: in-process A/B host RAM for the 2nd leg may include allocator-retained pages from the 1st;"
        );
        println!("      use MACH_FP8=0 / MACH_FP8=1 single-leg runs for clean host-RAM numbers.");
    }
    if let (Some(a), Some(b)) = (&f16, &fp8) {
        ...
        println!("  max|f16 - FP8| = {max:.6} (scale {scale:.3}) | greedy argmax match: {same}");
        println!("  -> FP8 only changes storage precision; compute runs the identical f16 path.");
    }
}
...
fn bench_ttft_fp8(
    ...
    let prompt: Vec<u32> = (0..prompt_len).map(|i| (i % 977) as u32).collect();
    let mut eng =
        ContinuousModel::with_prefill_rows_fp8(hip.clone(), cfg, w, capacity, capacity).unwrap();
    let id = eng
        .add(
            &prompt,
            16,
...
fn bench_tpot_fp8(
    ...
    use mach_model::batched::BatchedModel;
    let mut bm = BatchedModel::from_fp8(hip.clone(), cfg, w, batch).unwrap();
    let seq: Vec<u32> = (0..batch).map(|i| (i % 977) as u32).collect();
    bm.reset_state().unwrap();
    for _ in 0..3 {
        bm.decode_step(&seq).unwrap();
...
#[cfg(not(feature = "hip"))]
fn main() {
    eprintln!("fp8_bench requires the `hip` feature");
}
