...
// Request/response types and pure prompt helpers live in `crate::chat`
// (compiled without `hip`); re-exported so `crate::routes::…` paths and the
// public `mach_server::ChatFormat` re-export keep working.
pub use crate::chat::{
    ChatFormat, ChatMessage, ChatRequest, CompletionChoice, CompletionRequest, CompletionResponse,
    Logprobs, MAX_CHAT_BODY_BYTES, Prompt, StopSpec, TopLogprob, Usage,
};
use crate::chat::{
    chat_body_limit, chat_eos_text, chat_text, decode_text, logit_bias_pairs, naive_encode, now,
    prompt_tokens, remaining_patch_budget, stop_seqs,
};
