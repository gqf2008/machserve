...
| **Qwen3.8-27B Q4-all prefill(4020-token prompt,真机,2026-09-11)** | **51.3s ≈79 tok/s(Q4 行复用 + LM head 同分派后 ~1.9x;此前 98.2s ≈41 tok/s)** |
...
- **分页 KV + 跨请求前缀共享(实验)**:SHA-256 前缀哈希链 + LCM 块池 + 复用规划 +
  ...
  GQA + online-softmax；MLA paged 仍保留 legacy full-score 路径，且 server 的
...
#     MACH_VISION_MAX_TOKENS 内再服务；默认关闭，保持超预算图 400)、
